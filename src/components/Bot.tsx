  ...
  const previewDisplay = (item: FilePreview) => {
    return (
      <>
        ...
        <Show when={item.mime.startsWith('audio/')}>
          <div
        ...
        <Show when={!item.mime.startsWith('image/') && !item.mime.startsWith('audio/')}>
          <FilePreview item={item} onDelete={() => handleDeletePreview(item)} />
        </Show>
      </>
    );
  };
  ...
  return (
    <>
      ...
      >
        ...
        {props.showTitle ? (
          <div
            class="flex justify-between items-center w-full h-[70px] px-3 absolute top-0 left-0 z-10"
            style={{
        ...
        <div class="flex flex-col w-full h-full justify-start z-0">
          <div
            ref={chatContainer}
            class="overflow-y-scroll flex flex-col flex-grow min-w-full w-full px-3 pt-[70px] mt-2 relative scrollable-container chatbot-chat-view scroll-smooth gap-4"
          >
            <For each={[...messages()]}>
              {(message, index) => {
                return (
